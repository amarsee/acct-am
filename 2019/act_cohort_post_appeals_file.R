...
grad_rate_student_level <- read_csv("N:/ORP_accountability/data/2019_graduation_rate/student_level.csv") %>% 
...
junior_2_prior <- read.dta13("N:/Assessment_Data Returns/ACT/2016-17/Junior Day File/20170713_ACT_JuniorDayResults_SY2016-17_Whalen_v1.dta") %>% 
  ...
  mutate(test_date = '2016-17', pref = 4)


# ============= Junior Day Prior ====================
junior_prior <- read.dta13("N:/Assessment_Data Returns/ACT/2017-18/2018 Spring/Final Spring Files/20180716_ACT_JuniorDayResults_SY2017-18_Whalen_v1.dta") %>% 
  filter(test_location != "M") %>% 
  # dedup by highest composite
  group_by(state_stud_id) %>% 
  mutate(temp = max(act_composite)) %>% 
  ...
  mutate(cr_all = if_else(cr_eng == 1 & cr_math == 1 & cr_read == 1 & cr_sci == 1, 1, 0)) %>% 
  select(unique_student_id = state_stud_id, act_eng:act_composite, cr_eng:cr_sci, cr_all) %>% 
  mutate(test_date = '2017-18', pref = 3)

# ============= Junior Day Current ====================
junior_current <- read_csv("N:/Assessment_Data Returns/ACT/2018-19/2019 Spring/Spring 2019 Final File/20190620_ACT_JuniorDayResults_SY2018-19.csv") %>% 
  filter(test_location != "M") %>% 
  # dedup by highest composite
  group_by(state_stud_id) %>% 
  mutate(temp = max(act_composite)) %>% 
  ...
  mutate(test_date = '2018-19', pref = 2)

# ============= Cohort Highest Score File ====================
cohort_highest_score <- read_csv("N:/Assessment_Data Returns/ACT/2018-19/2019 Graduation Cohort Final/2019 Highest score files/20190916_ACT_GraduatingCohortHighestScore_SY2018-19.csv") %>% 
  # dedup by highest composite
...
# write_csv(act_student_level_post_appeals, "N:/ORP_accountability/data/2019_ACT/ACT_student_post_appeals_AM.csv", na = '')
