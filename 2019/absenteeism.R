...
write_csv(attendance, "N:\\ORP_accountability\\data\\2019_chronic_absenteeism\\absenteeism_pull_Jul11.csv")

attendance_old <- read_csv("N:/ORP_accountability/data/2019_chronic_absenteeism/absenteeism_pull.csv")

diff_df <- dplyr::setdiff(attendance_old %>% select(-begin_date, - end_date), attendance %>% select(-begin_date, - end_date)) %>% 
  dplyr::setdiff(attendance %>% select(-begin_date, - end_date), attendance_old %>% select(-begin_date, - end_date))
...
write_csv(student, str_c("student_chronic_absenteeism_AM_", month(today(), label = TRUE), day(today()), ".csv"), na = "")
...
student %>%
  ...
  walk2(
    .x = ., 
    .y = district_numbers, 
    .f = ~ write_csv(.x, path = paste0("N:/ORP_accountability/data/2019_chronic_absenteeism/split/", .y, "_ChronicAbsenteeismStudentFile_9Jul2019.csv"), na = "")
  )
