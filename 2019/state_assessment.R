...
state_assessment_prior <- read_csv("N:/ORP_accountability/data/2018_final_accountability_files/state_assessment_file.csv")
...
state_assessment <- state_totals %>% 
  ...
  bind_rows(state_assessment_prior %>% filter(year %in% c(2018, 2017), subject %in% unique(state_totals$original_subject))) %>% 
...
# Write csv
write_csv(state_assessment, "N:/ORP_accountability/data/2019_final_accountability_files/state_assessment_file_AM.csv")
