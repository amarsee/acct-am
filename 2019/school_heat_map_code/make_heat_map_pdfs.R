...
for (dist in district_list) {
  ...
  for(school_index in ind_list) {
    ...
    if(pasted_dist_school %in% schools_to_update$system_school){
      ...
      # school_filename <- paste0(dist_pools[[school_index]], '_', school_filename_joined,
      #                           '_', dist_school_nos[[school_index]], '_2019.pdf')
      # school_filename <- paste0(dist_pools[[school_index]], '_', 'schoolheatmapsfile',
      #                           '_', school_filename_joined, '_', dist_school_nos[[school_index]], '_2019.pdf')
      school_filename <- paste0(dist, '_', 'SchoolHeatMapsFile',
                                '_', school_filename_joined, '_', dist_school_nos[[school_index]], '_',
                                as.character(month(Sys.Date(), label = TRUE)), as.character(day(Sys.Date())), '.pdf')
