...
# ======================== School Level ============================
school_success <- read_csv("N:/ORP_accountability/projects/2021_amo/success_rate_targets_school.csv") %>% 
...
school_ready_grad <- read_csv("N:/ORP_accountability/projects/2021_amo/ready_grad_targets_school.csv") %>% 
  transmute(system, system_name, school, school_name, indicator = 'Ready Graduate', subgroup,
            grade = '9th through 12th', n_students = grad_count_prior, prior_metric = pct_ready_grad, 
            amo_target = AMO_target, amo_target_double = AMO_target_double)

school_abs <- read_csv("N:/ORP_accountability/projects/2021_amo/absenteeism_targets_school.csv") %>% 
...
school_combined %>%
  ...
  walk2(
    ...
    .f = ~ write_csv(.x, path = paste0(
      ...
      "_2020_SchoolLevelAMO_", sprintf("%02d", day(now())), month(now(), label = T), year(now()),
      ".csv"
    ), na = "")
  )

# ================================ District Level ================================
district_abs <- read_csv("N:/ORP_accountability/projects/2021_amo/absenteeism_targets_district.csv") %>% 
  transmute(system, system_name, indicator = 'Chronic Absenteeism', subgroup,
            grade = 'All Grades', n_students, prior_metric = pct_chronically_absent, 
            amo_target = AMO_target, amo_target_double = AMO_target_double)

district_elpa <- read_csv("N:/ORP_accountability/projects/2021_amo/elpa_targets_district.csv") %>% 
...
district_success <- read_csv("N:/ORP_accountability/projects/2021_amo/success_rate_targets_district.csv") %>% 
...
district_combined %>%
  ...
  walk2(
    ...
    .f = ~ write_csv(.x, path = paste0(
      "N:/ORP_accountability/projects/", year(now())+1, "_amo/split/", .y,
      "_2020_DistrictLevelAMO_", sprintf("%02d", day(now())), month(now(), label = T), year(now()),
      ".csv"
    ), na = "")
  )
