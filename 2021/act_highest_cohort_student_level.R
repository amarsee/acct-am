...
appeals_tracker <- read_excel("N:/ORP_accountability/appeals/2021/ACT/Coding/ACT Appeals Master Tracker.xlsm", sheet = 3) %>% 
  ...
  mutate(
    student_key = as.numeric(student_id),
    remove = if_else(!act_english_highest %in% '0':'36', 1, 0),
    act_english_highest = if_else(!act_english_highest %in% '0':'36', '0', act_english_highest)
  ) %>% 
...
act_student_w_appeals <- act_student_level %>% 
  # mutate(sat_total = NA) %>% 
  left_join(appeals_tracker %>% select(student_key, act_english_highest:act_composite_highest, cr_eng_app:cr_all_app, sat_total, remove),
            by = 'student_key') %>% 
  mutate(
...
act_student_level_post_appeals <- act_student_w_appeals %>% 
  mutate(remove = if_else(student_key == 3586177, 1, remove)) %>% 
  ...
  select(-(cr_eng:cr_all), -remove)

# write_csv(act_student_level_post_appeals, "N:/ORP_accountability/data/2020_ACT/ACT_student_post_appeals.csv", na = '')


# ============= Retake =================
grad_cohort <- read_csv("N:/ORP_accountability/data/2020_graduation_rate/student_level.csv") %>% 
  filter(included_in_cohort == 'Y')

retake <- read_csv("N:/Assessment_Data Returns/ACT/2019-2020/2019 Fall Testing/Senior Retake Preliminary Files/20200831_ACT_cohort_retake_SY2019-20.csv") %>% 
  filter(test_location != "M") %>% 
  # dedup by highest composite
  group_by(state_id) %>% 
  mutate(temp = max(act_composite_scale)) %>% 
  filter(act_composite_scale == temp) %>% 
  # dedup by max math
  mutate(temp = max(act_math_scale)) %>% 
  filter(act_math_scale == temp) %>% 
  # dedup by max reading
  mutate(temp = max(act_read_scale)) %>% 
  filter(act_read_scale == temp) %>% 
  # dedup by max english
  mutate(temp = max(act_eng_scale)) %>% 
  filter(act_eng_scale == temp) %>% 
  # dedup by max science
  mutate(temp = max(act_sci_scale)) %>% 
  filter(act_sci_scale == temp) %>% 
  ungroup() %>% 
  # mutate(cr_all = if_else(cr_eng == 1 & cr_math == 1 & cr_read == 1 & cr_sci == 1, 1, 0)) %>% 
  # select(unique_student_id = state_id, act_eng:act_composite, cr_eng:cr_sci, cr_all) %>% 
  mutate_at(
    .vars = vars(act_eng_scale:act_composite_scale),
    .funs = as.integer
  )
  # mutate(test_date = '2018-19', pref = 3)
...
# improvement
act_retake_junior_day <- junior_prior %>% 
  inner_join(
    retake %>% 
      mutate(state_id = as.numeric(state_id)) %>% 
      select(
        unique_student_id = state_id,
        act_eng_scale:act_composite_scale
      ),
    by = "unique_student_id"
  ) %>% 
  filter(unique_student_id %in% grad_rate_student_level$student_key,
         !is.na(act_composite_scale)) %>% 
  mutate(
    improvement = if_else(act_composite_scale > act_composite, 1, 0)
  )

# Number took both
n_distinct(act_retake_junior_day$unique_student_id)

# Number improved
sum(act_retake_junior_day$improvement)

# Average on junior day
mean((junior_prior %>% filter(unique_student_id %in% grad_rate_student_level$student_key))$act_composite)
n_distinct((junior_prior %>% filter(unique_student_id %in% grad_rate_student_level$student_key))$unique_student_id)
# Average of all students in junior day
mean((junior_prior)$act_composite)
n_distinct((junior_prior)$unique_student_id)
# Mean of students who took both
mean(act_retake_junior_day$act_composite)
# Mean retake
mean(act_retake_junior_day$act_composite_scale)
