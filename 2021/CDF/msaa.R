...
math <- readxl::read_excel("N:/Assessment_Data Returns/TCAP ALT_ Grades 3-11_MSAA/2020-21/2021_TN_StateStudentResults.xlsx") %>%
    transmute(
        system = as.integer(DistrictID),
        school = as.integer(SchoolID),
        unique_student_id = as.numeric(State_Student_ID),
...
ela <- readxl::read_excel("N:/Assessment_Data Returns/TCAP ALT_ Grades 3-11_MSAA/2020-21/2021_TN_StateStudentResults.xlsx") %>%
    transmute(
        system = as.integer(DistrictID),
        school = as.integer(SchoolID),
        unique_student_id = as.numeric(State_Student_ID),
...
# Demographic file
demographics <- read_csv("N:/ORP_accountability/projects/Andrew/Demographics/Data/student_demographics_06082021.csv") %>%
...
names <- read_csv("N:/ORP_accountability/data/2021_final_accountability_files/names.csv")

msaa <- bind_rows(math, ela) %>%
  mutate(
    # All 963 (School for Blind) records have an inactive school number
    # It should be 5
    school = if_else(system == 963 & school == 10, 5L, school)
  ) %>% 
...
write_csv(msaa, "N:/ORP_accountability/data/2021_cdf/2021_msaa_cdf.csv", na = "")
